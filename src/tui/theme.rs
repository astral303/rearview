...
pub struct Theme {
    ...
    pub green: Rgb,
    pub blue: Rgb,
    /// One colour per call issued together, by position in the batch; the
    /// connector, rule and tool word of each. Repeats past the end.
...
pub fn detect_theme() -> &'static Theme {
    THEME.get_or_init(|| {
        // terminal-light writes its query to stdout, so a probe while stdout
        // is redirected puts escape bytes into captured output such as
        // `$(rearview --show-id)`. The probe also puts the terminal into raw
        // mode, and a process in a background process group blocks
        // indefinitely in `tcsetattr` while doing so. The pager path keeps
        // stdout on the terminal, so detection still applies there. Tests
        // never probe, which keeps them independent of the terminal that
        // launched them.
